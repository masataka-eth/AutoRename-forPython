...
root = tkinter.Tk()
root.title('Auto Rename Folder All')
...
def SelectFolder():
    try:
        global targetfolder
        dir = 'C:\\Users\\mayuzumi\\Desktop\\Parent folder_artengine'
...
def ExeOutput():
    try:
        ...
        with os.scandir(targetfolder) as it:
            for entry in it:
                if entry.is_dir():
                    setfolder = targetfolder + "/" + entry.name
                    files = sorted(glob.glob(setfolder + "/*"), key=natural_keys)
                    ...
                    for file in files:
                        print(file)
                        os.rename(file, setfolder + '/' + datalist[i] + extension)
                        i+=1
                    var2.set(entry.name + "Success Complete!")
    except:
        var2.set("Exception error occurred!")
